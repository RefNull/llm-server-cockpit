...
4. **Audit & Enforced Contracts**:
   ...
   - The 8 tables (Dashboard renders no table; `builds-table` no longer exists — retained builds moved into `RetainedBuildsModal`):
     - `backends-table` (`builds.py`): 5 columns ("Component" w=24, "Installed" w=14, "Status" w=35, `Build`/`Rebuild` action w=11, `Info` action w=8). Render 102. Component is w=24, not w=22, because a stock recipe renders an `EX. ` prefix (`manifest.yaml` `backends.<name>.example`) and `"EX. llama.cpp (vulkan)"` is exactly 22 cells — w=22 would fit today with zero slack and clip the first longer backend name. **Installed** is what `prefix_root/<backend>/current` actually points at — per backend, styled green when it matches the selected pin, yellow when it differs, dim when absent — and **Status** is the upstream comparison. **The pin is deliberately NOT a column.** It lives on a `Static` line above the table, because `manifest.yaml`'s `llama_cpp.ref` is one global value: rendering it into every row made "Update to latest" read as "changes every row", which is what an operator reported on 2026-09-18. A global fact in a per-row column is a lie about the shape of the data — hoisting it is the fix, and it is what freed the 14 cells this table's `Info` action now spends. The `Build` column is w=11, not `len("Build")+4=9`, because a callable label must fit its *widest* resolved value, `"Rebuild"` (§9, state toggles). `Info` opens `BackendDetailModal`, which answers the questions that report raised in one place: which GPU in the host profile declares this backend, where it installs, every retained ref, and the resolved cmake invocation — obtained from `provision.steps.build.resolve_cmake_argv`, **the same function the build itself calls**, so the displayed recipe cannot drift from the one that runs. Its `cmake_flags` are editable there, written by the list-block equivalent of the pin's targeted line rewrite.
