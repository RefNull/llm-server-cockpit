...
- **GPU telemetry is never automatic.** `metrics.read_gpus()` (nvidia-smi/xpu-smi) wakes the device and, on an Intel Arc, keeps sysman telemetry active — the fans ramp to 100% within moments. The Dashboard is the landing tab, so an automatic read means every launch of the cockpit pokes every GPU. It has been fixed twice: commit 5c986a4 removed a 1Hz sampler, and the mount-time read was removed after QA reported the ramp again on plain launch. The only caller is the explicit `Sample GPUs` control (`g`), and the GPU bars stay hidden behind "not sampled" until then — a 0% bar would be a made-up reading (decision 0d.4). This is the one exception to "zero controls on the Dashboard", and it is a read, not a mutation. CPU/RAM stay automatic: /proc wakes nothing. Enforced by `smoke/verify_no_gpu_wake.py`, at the subprocess boundary so a new caller reaching a vendor tool some other way is caught too.
