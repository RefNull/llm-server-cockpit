...
0. **A hidden tab does no I/O**:
   ...
   - **`on_tab_shown()` is the per-view sibling of `on_first_view()`** — it fires *every* time a screen becomes the visible tab, including the first, and is never latched. `CockpitApp._load_visible_screens` calls it immediately after `ensure_first_view()` on every `TabbedContent.TabActivated`. Use it for something that must be *re-checked* on return (the Backends tab's 15-minute upstream check) and `on_first_view` for a one-time load. Anything it does must be cache-gated or free: it runs on every tab switch, which is exactly the traffic §3.0 exists to prevent.
...
4. **Audit & Enforced Contracts**:
   ...
   - The 8 tables (Dashboard renders no table; `builds-table` no longer exists — retained builds moved into `RetainedBuildsModal`):
     - `backends-table` (`builds.py`): 8 columns ("GPU" w=6, "Backend" w=9, "Active build" w=15, "Status" w=18, `Build` action w=9, `Prebuilt` action w=12, `Edit` action w=8, `Historical builds` action w=21). Render 114 (strictly <= 115 budget). **`Status` is per-backend build staleness, never the global upstream check** — `not built` / `up to date` (green) / `pinned at <ver>` (yellow, the operator activated an older build while one at the pin exists) / `out of date, built on <ver>` (red). It used to render `self._llama_cpp_check` verbatim on every row: a duplicate of the section line above the table, a global value in a per-row column, and silent on the only per-row question (QA 2026-09-18). **`Active build` carries no colour** — it is an identity, and colouring it too would make two adjacent columns encode one verdict. Every version string named in a row is that row's own build, never the pin; the pin appears once, in the Llama.cpp section line.
     ...
     - `scripts-table` (`scripts.py`): 9 columns ("ID" w=14, "Type" w=7, "Path" w=17, "Status" w=13, `run` toggle w=9, `Logs` action w=8, `boot` toggle w=11, `Edit` action w=8, `Remove` action w=10). Render 115.
     ...
     - **The Installs tab's two `.section-title` sections are the layout contract** (`builds.py`, plans/06 Phase 2): *Llama-swap* is `.form-row` field:value lines and an update action, **never a table** — it used to be a row in `backends-table` whose Build and Edit cells were permanently blank, because it is not in `self.backends`. *Llama.cpp* mirrors that shape above its own table. The pinned version lives in section two's form rows and **must not return to a column**: it is one global value, so rendering it per row made "Update to latest" read as "changes every row" (QA 2026-09-18). Nothing on this tab may carry free-standing explanatory prose — two such notes were deleted in the same pass, and the habit of answering a question by adding a paragraph is what the operator called "ballooned".
     - `builds-list-table` (`builds.py` `BuildsListModal`, 90%-width dialog, scoped to one backend): 4 columns ("Build" w=10, "Status" w=13, "Version" w=10, "When" w=16) + 4 action columns (`Activate` w=12, `Config` w=10, `Log` w=7, `Remove` w=10). Render 104, against a **measured** 114-cell dialog at 121×30 — this is the entry that proves why §4.4 says modal tables are budgeted against their dialog: a first pass carried a separate "Outcome" column, rendered 116, and put `Remove` out of reach while passing every other gate, because `_assert_table_widths_in_budget` measures on-screen tables against 115 and never sees a modal. `Status` and `Outcome` were merged because "NOT SANE" and "build_failed" never disagree. Guarded by `_assert_builds_list_modal` in `smoke/verify_screens.py`, which asserts render ≤ the real dialog width at `-wide` only (§4.2 permits horizontal scroll below the breakpoint). It replaces `history-table` and `retained-table`, both removed with their modals.
