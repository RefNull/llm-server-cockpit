...
```
- **Threshold Rationale (110 cells)**: Grounded in `#dashboard-columns` (side-by-side metric and hardware status panels requiring ~50 cells each + borders/gutters = ~110 cells; guarantees clearance without wrapping gauge readouts or clipping panel titles). Settings was previously derived for two columns but was refactored upstream into `TabbedContent` (`#settings-tabs`), leaving Dashboard as the primary multi-column layout.
...
5. **Two-Column Layout Threshold**:
   - Multi-column layouts (such as `#dashboard-columns`) are permitted only when the `Screen.-wide` class is active (terminal width ≥ 110).
   - Under `Screen.-narrow` (< 110 cells), multi-column containers must collapse to a single vertical stack (`layout: vertical`) via `.columns-responsive`.
...
3. **Audit & Enforced Contracts**:
   ...
   - All 9 tables across 6 screens (Dashboard renders no table):
     - `backends-table` (`builds.py`, tick-able, `fixed_columns=2`): 4 columns ("" w=3, "Component" w=22 pinned, "Pinned" w=12, "Update" w=34).
     - `builds-table` (`builds.py`, `fixed_columns=1`): 4 columns ("Backend" w=12 pinned, "Ref" w=14, "Status" w=12, "Integrity" w=12).
     - `history-table` (`builds.py`, `fixed_columns=1`): 4 columns ("Backend" w=12 pinned, "Timestamp (UTC)" w=22, "Outcome" w=16, "Detail" w=40).
     ...
     - `gpu-table` (`settings.py`, `fixed_columns=1`): 3 columns ("GPU ID" w=16 pinned, "Vendor" w=16, "Backends" w=30). Composed in both the first-run wizard and the normal-mode Hardware sub-tab — only one branch is ever mounted, with identical columns.
...
All 19 modal confirmation call sites across the 6 mutating screens (Dashboard is read-only and opens no modal):
- **Builds** (`builds.py`):
